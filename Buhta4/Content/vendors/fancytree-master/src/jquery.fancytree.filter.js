...
; (function ($, window, document, undefined) {
        ...
        if (typeof filter === "string") {
            ...
            filter = function (node) {
                ...
                if (res && filterOpts.highlight) {
                    ...
                    if (node.saveHtml)  // buhta
                        node.titleWithHighlight = node.saveHtml.replace(node.title, node.titleWithHighlight); // buhta
    ...
    $.ui.fancytree._FancytreeClass.prototype.clearFilter = function () {
        this.visit(function (node) {
            if (node.match) {  // #491
                if (node.saveHtml)  // buhta
                    $(">span.fancytree-title", node.span).html(node.saveHtml); // buhta
                else // buhta
